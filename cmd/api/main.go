...
import (
	...
	"bitbucket.ideasoft.io/dex/dex-backend/services/api"
...
func main() {
	...
	log := logrus.New()
	logger := log.WithField("service", "api")
	...
	service := api.NewApiService(logger, &cfg, qi, redisClient, ethClient, contractClient)
	service.Run(ctx)
}
